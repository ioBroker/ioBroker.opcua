...
function OPCUAClient(adapter, options, onOnlyTestConnection) {
    ...
    this.getSubscribes = function () {
        ...
        Object.keys(states).map(
            id => (subscribers[states[id].native.nodeId] = { fullPath: states[id].native.fullPath, id }),
        );
        ...
        if (onOnlyTestConnection) {
            ...
            subSession
                .on('started', () => {
                    ...
                    that.subscribeStates(() => onConnectChanged(true));
                })
    ...
    function tests() {
        that.browse('ns=0;i=63').then(result => {
    ...
    this.subscribeStates = function (cb) {
        ...
        if (id) {
            this._subscribe(id).then(() => setImmediate(() => this.subscribeStates(cb)));
        } else {
            cb && cb();
        }
    };
